...
class ModelWrapper:
    ...
    def get_log_probs(
            self, tokens: List[str], softmax_t: float = 1.0,
            ) -> torch.Tensor:
        ...
        softmax_t is softmax temperature.
        """
        ...
        with torch.no_grad():
            ...
            return torch.log_softmax(logits * softmax_t, dim=1)
    ...
    def get_occurred_log_probs(
        ...
        """
        log_probs = self.get_log_probs(tokens, softmax_t=softmax_t)
        out = []
    ...
    def get_next_top_k(
            self, tokens: List[str], top_k: int, softmax_t: float = 1.0,
            ) -> List[Tuple[float, str]]:
        """ Return a list of top k tuples of log prob and token,
        for what would come after the last token.
        """
        next_log_probs = self.get_log_probs(tokens, softmax_t=softmax_t)[-1]
